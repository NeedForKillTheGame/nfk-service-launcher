...
Позволяет запускать, останавливать сервер, изменять/отправлять определенные файлы - через простой API интерфейс.
